...
// two bidi rules in index.css — so the only way to know a Hebrew line came out
...
const results = await page.evaluate((cases) => {
  ...
  for (const item of cases) {
    ...
    for (const el of document.querySelectorAll('.view-line')) {
      ...
      for (let node = walker.nextNode(); node; node = walker.nextNode()) {
        for (let i = 0; i < node.data.length; i++) {
          ...
          if (box.width || box.height) order.push({ ch: node.data[i], x: box.left });
        }
      }
      ...
      lines.push({
        ...
        // character to the reader and to the bidi algorithm.
        visual: order.map(o => o.ch).join('')
          .replace(/\u00a0/g, ' ')
          // The marks themselves take no room; a placeholder box would not be
          // stripped here, which is how a regression to those gets caught.
          .replace(/[\u200e\u200f\u061c\u202a-\u202e\u2066-\u2069]/g, ''),
        top: box.top,
    ...
    out.push(lines.map(({ visual, fromLeft, fromRight }) => ({ visual, fromLeft, fromRight })));
    model.dispose();
  }
  return out;
