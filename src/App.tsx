...
const PHYSICS_EQS: { group: string, name: string, physica?: boolean, code: string }[] = [
  ...
  { group: 'QCD', name: 'QCD Lagrangian', code: 'cal(L)_"QCD" = sum_f macron(psi)_f (i gamma^mu D_mu - m_f) psi_f - 1/4 G^a_(mu nu) G_a^(mu nu)' },
  { group: 'QCD', name: 'Gluon field strength', code: 'G^a_(mu nu) = partial_mu A^a_nu - partial_nu A^a_mu + g_s f^(a b c) A^b_mu A^c_nu' },
  { group: 'QCD', name: 'Covariant derivative (fundamental & adjoint)', code: 'D_mu = partial_mu - i g_s A^a_mu t^a, quad quad (D_mu)^(a b) = delta^(a b) partial_mu + g_s f^(a c b) A^c_mu' },
  { group: 'QCD', name: 'Colour algebra & Casimirs', code: '[t^a, t^b] = i f^(a b c) t^c, quad tr(t^a t^b) = T_F delta^(a b), quad C_F = (N^2 - 1)/(2 N), quad C_A = N' },
  ...
  { group: 'QCD', name: 'Factorisation for a hadronic cross section', code: 'dif sigma = sum_(a, b) integral dif x_1 dif x_2 space f_a (x_1, mu_F^2) f_b (x_2, mu_F^2) space dif hat(sigma)_(a b) (x_1 x_2 s, mu_F^2)' },
