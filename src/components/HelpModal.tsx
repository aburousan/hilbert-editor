...
const SECTIONS: Section[] = [
  {
    ...
    features: [
      { title: 'Command Palette', where: 'Edit menu, or anywhere', keys: keys('⌘K'), desc: 'Search and run every command in the app — inserts, tools, export, settings — without hunting through menus.' },
      { title: 'Switch between open files', where: 'Anywhere', keys: keys('Ctrl+Tab · ⌘/Ctrl+PageUp/PageDown · ⌘1…⌘9'), desc: 'Step through the open tabs, or jump straight to one by number — ⌘9 is always the last. Shift reverses the direction, and ⌘⌥←/→ does the same on macOS. The tab you land on scrolls itself into view.' },
      ...
      { title: 'Find & Replace', where: 'Edit menu', keys: keys('⌘F'), desc: 'Monaco search with regex and replace-all across the open file.' },
      ...
      { title: 'Formatting', where: 'Formatting menu / toolbar', keys: keys('⌘B · ⌘I'), desc: 'Bold, italic, underline, sub/superscript, small caps, colours, highlights, alignment, rotation, letter spacing, boxes.' },
      ...
      { title: 'Comment lines', where: 'Edit menu, or the command palette', keys: keys('⌘/ · Ctrl+/'), desc: 'Comment or uncomment the current line, or every line in the selection. Uses the comment syntax of the file you are in — // in Typst, # in Python and Julia, % in a .bib.' },
      ...
      { title: 'Customize toolbar', where: 'Edit → Customize Toolbar', desc: 'Show only the shortcut buttons you use. Hidden actions remain available from menus, the command palette and their keyboard shortcuts.' },
      ...
      { title: 'Zoom the preview', where: 'PDF preview', keys: keys('⌘/Ctrl + scroll'), desc: 'Hold Ctrl (⌘ on macOS) and scroll — or pinch on a trackpad — to zoom around the pointer. The toolbar’s +/−, a zoom menu and Fit width are still there.' },
    ],
  },
  {
    name: 'Mathematics',
    features: [
      { title: 'Equations', where: 'Insert → Math', keys: keys('⌘E · ⌘⇧E'), desc: 'Inline, block, multiline/aligned and numbered equations; toggle numbering per equation or document-wide.' },
      { title: 'Equation templates', where: 'Insert → Math', keys: keys('⌘⇧G'), desc: 'A gallery of ready-made structures — integrals, sums, derivatives, brackets — inserted as editable Typst.' },
      ...
      { title: 'Matrix Studio', where: 'Insert → Math', keys: keys('⌘⇧M'), desc: 'Visual matrix builder: fill cells, add augmentation lines and borders, get clean Typst array code.' },
      { title: 'Symbols', where: 'Insert → Math', keys: keys('⌘⇧P'), desc: 'Searchable maths & physics symbol picker. Or draw a symbol with the mouse and let the app find it (⌘⇧Y).' },
      { title: 'Compute selection', where: 'Insert → Math', keys: keys('⌘⇧U'), desc: 'Select an expression and simplify or solve it in place using a computer algebra backend.' },
    ],
  },
  {
    name: 'Figures & Diagrams',
    features: [
      { title: 'Feynman diagrams', where: 'Insert → Plots', keys: keys('⌘⇧F'), desc: 'Visual builder: propagators (fermion, photon, gluon, scalar, ghost…), loops with fermion-flow arrows, vertices, labels — emits editable cetz code.' },
      ...
      { title: 'Flowcharts', where: 'Insert → Compute', keys: keys('⌘⇧L'), desc: 'Draw a flowchart and get runnable code logic from it, or a fletcher flow diagram for the document.' },
  ...
  },
  {
    ...
    features: [
      ...
      { title: 'Code runner', where: 'Insert → Compute', keys: keys('⌘⇧K'), desc: 'Scratchpad for Python, Julia and Wolfram with live output; insert results or plots into the document.' },
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
                  {s.features.map(f => (
                    <div key={f.title} style={{ display: 'flex', gap: 12, alignItems: 'baseline' }}>
                      ...
                      <div style={{ flex: 1, fontSize: '0.8rem', color: 'var(--text-muted)', lineHeight: 1.45 }}>{keys(f.desc)}</div>
                    </div>
                  ))}
                </div>
              </div>
